...
use LND_file_parser, only : open_param_file, param_file_type
use LND_io, only : file_exists, close_file, slasher, ensembler
use LND_io, only : open_namelist_file, check_nml_error

implicit none ; private

public get_LND_input
...
subroutine get_LND_input(param_file, dirs, check_params, default_input_filename, ensemble_num)
  ...
  namelist /LND_input_nml/ output_directory, input_filename, parameter_filename, &
  ...
  if (file_exists('input.nml')) then
    ...
    call LND_error(FATAL,'Required namelist file input.nml does not exist.')
  endif

  ! Read namelist parameters
  ierr=1 ; do while (ierr /= 0)
    read(unit, nml=LND_input_nml, iostat=io, end=10)
    ierr = check_nml_error(io, 'LND_input_nml')
  enddo
10 call close_file(unit)
  ...
  if (present(param_file)) then
    ...
    enddo
    if (valid_param_files == 0) call LND_error(FATAL, "There must be at "//&
         "least 1 valid entry in input_filename in LND_input_nml in input.nml.")
  endif

end subroutine get_LND_input

end module LND_get_input
