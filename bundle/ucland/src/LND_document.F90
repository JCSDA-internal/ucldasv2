...
use LND_time_manager,  only : time_type, operator(==), get_time, get_ticks_per_second
...
subroutine open_doc_file(doc)
  ...
  if ((len_trim(doc%docFileBase) > 0) .and. doc%complete .and. (doc%unitAll<0)) then
    ...
    if ((.not.opened) .or. (ios /= 0)) then
      call LND_error(FATAL, "Failed to open doc file "//trim(fileName)//".")
    endif
...
! Returns >0 if found. FATAL if not.
  ...
  do find_unused_unit_number=512,42,-1
    ...
    if (.not.opened) exit
  enddo
  if (opened) call LND_error(FATAL, &
...
!!if (mesg(1:1) == '!') return ! Ignore commented parameters
  ...
  do while( associated(this) )
    if (trim(doc%blockPrefix)//trim(varName) == trim(this%name)) then
      ...
      call LND_error(WARNING, "Previous msg:"//trim(this%msg))
      call LND_error(WARNING, "New message :"//trim(mesg))
      call LND_error(WARNING, "Encountered inconsistent documentation line for parameter "&
                     //trim(varName)//"!")
    endif
    last => this
    this => this%next
