!> The UCLAND facility to parse input files for runtime parameters
module LND_file_parser

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_coms, only : root_PE, broadcast
use LND_error_handler, only : LND_error, FATAL, WARNING, LND_mesg
use LND_error_handler, only : is_root_pe, stdlog, stdout
use LND_time_manager, only : get_time, time_type, get_ticks_per_second
use LND_time_manager, only : set_date, get_date, real_to_time, operator(-), set_time
use LND_document, only : doc_param, doc_module, doc_init, doc_end, doc_type
use LND_document, only : doc_openBlock, doc_closeBlock
use LND_string_functions, only : left_int, left_ints, slasher
use LND_string_functions, only : left_real, left_reals
...
subroutine open_param_file(filename, CS, checkable, component, doc_file_dir)
  ...
  character(len=*), optional, intent(in) :: component   !< If present, this component name is used
                                         !! to generate parameter documentation file names; the default is"LND"
  ...
  if (strlen == 0) then
    call LND_error(FATAL, "open_param_file: Input file has not been specified.")
  endif
  ...
  if (.not.file_exists) call LND_error(FATAL, &
  ...
  if (Netcdf_file) &
    call LND_error(FATAL,"open_param_file: NetCDF files are not yet supported.")
  ...
  if (all_PEs_read .or. is_root_pe()) then
    ...
    enddo
    if (iounit >= 512) call LND_error(FATAL, &
    ...
    if (ios /= 0) call LND_error(FATAL, "open_param_file: Error opening "// &
                                       trim(filename))
  else
    iounit = 1
  endif
  ...
  call LND_mesg("open_param_file: "// trim(filename)// &
...
# include "version_variable.h"
  ...
  if (present(quiet_close)) then ; if (quiet_close) then
    do i = 1, CS%nfiles
      ...
      call LND_mesg("close_param_file: "// trim(CS%filename(i))// &
  ...
  docfile_default = "LND_parameter_doc"
    ...
    if (is_root_pe() .and. (CS%report_unused .or. &
      ...
      do n=1,CS%param_data(i)%num_lines
        if (.not.CS%param_data(i)%line_used(n)) then
          ...
          if (CS%report_unused) &
            call LND_error(WARNING, "Unused line in "//trim(CS%filename(i))// &
  ...
  if (is_root_pe() .and. (num_unused>0) .and. CS%unused_params_fatal) &
    call LND_error(FATAL, "Run stopped because of unused parameter lines.")
...
subroutine populate_param_data(iounit, filename, param_data)
 ...
 8  continue ! get here when read() reaches EOF
    ...
    if (inMultiLineComment .and. is_root_pe()) &
      call LND_error(FATAL, 'LND_file_parser : A C-style multi-line comment '// &
...
18  continue ! get here when read() reaches EOF
    ...
    if (num_lines /= param_data%num_lines) &
      call LND_error(FATAL, 'LND_file_parser : Found different number of '// &
...
9 call LND_error(FATAL, "LND_file_parser : "//&
...
function simplifyWhiteSpace(string)
  ...
  if (insideString) then ! A missing close quote should be flagged
    if (is_root_pe()) call LND_error(FATAL, &
...
subroutine read_param_int(CS, varname, value, fail_if_missing)
  ...
  if (found .and. defined .and. (LEN_TRIM(value_string(1)) > 0)) then
    ...
    if (present(fail_if_missing)) then ; if (fail_if_missing) then
      if (.not.found) then
        call LND_error(FATAL,'read_param_int: Unable to find variable '//trim(varname)// &
                             ' in any input files.')
      else
        call LND_error(FATAL,'read_param_int: Variable '//trim(varname)// &
                             ' found but not set in input files.')
      endif
    endif ; endif
  endif
  return
 1001 call LND_error(FATAL,'read_param_int: read error for integer variable '//trim(varname)// &
...
subroutine read_param_int_array(CS, varname, value, fail_if_missing)
 ...
 991 return
    ...
    if (present(fail_if_missing)) then ; if (fail_if_missing) then
      if (.not.found) then
        call LND_error(FATAL,'read_param_int_array: Unable to find variable '//trim(varname)// &
                             ' in any input files.')
      else
        call LND_error(FATAL,'read_param_int_array: Variable '//trim(varname)// &
                             ' found but not set in input files.')
      endif
    endif ; endif
  endif
  return
 1002 call LND_error(FATAL,'read_param_int_array: read error for integer array '//trim(varname)// &
                             ' parsing "'//trim(value_string(1))//'"')
...
subroutine read_param_real(CS, varname, value, fail_if_missing, scale)
  ...
  if (found .and. defined .and. (LEN_TRIM(value_string(1)) > 0)) then
    ...
    if (present(fail_if_missing)) then ; if (fail_if_missing) then
      if (.not.found) then
        call LND_error(FATAL,'read_param_real: Unable to find variable '//trim(varname)// &
                             ' in any input files.')
      else
        call LND_error(FATAL,'read_param_real: Variable '//trim(varname)// &
                             ' found but not set in input files.')
      endif
    endif ; endif
  endif
  return
 1003 call LND_error(FATAL,'read_param_real: read error for real variable '//trim(varname)// &
                             ' parsing "'//trim(value_string(1))//'"')
...
991 continue
    ...
    if (present(fail_if_missing)) then ; if (fail_if_missing) then
      if (.not.found) then
        call LND_error(FATAL,'read_param_real_array: Unable to find variable '//trim(varname)// &
                             ' in any input files.')
      else
        call LND_error(FATAL,'read_param_real_array: Variable '//trim(varname)// &
                             ' found but not set in input files.')
      endif
    endif ; endif
  endif
  return
 1004 call LND_error(FATAL,'read_param_real_array: read error for real array '//trim(varname)// &
                             ' parsing "'//trim(value_string(1))//'"')
...
subroutine read_param_char(CS, varname, value, fail_if_missing)
  ...
  elseif (present(fail_if_missing)) then ; if (fail_if_missing) then
    call LND_error(FATAL,'Unable to find variable '//trim(varname)// &
                         ' in any input files.')
  endif ; endif
...
subroutine read_param_time(CS, varname, value, timeunit, fail_if_missing, date_format)
  ...
  if (found .and. defined .and. (LEN_TRIM(value_string(1)) > 0)) then
    ...
    if ((INDEX(value_string(1),'-') > 0) .and. &
      ...
      if (LEN_TRIM(err_msg) > 0) call LND_error(FATAL,'read_param_time: '//&
    ...
    elseif (INDEX(value_string(1),',') > 0) then
     ...
     995 continue
      if ((vals(1) < 0) .or. (vals(2) < 0) .or. (vals(3) < 0)) &
        call LND_error(FATAL,'read_param_time: integer list read error for time-type variable '//&
                       trim(varname)// ' parsing "'//trim(value_string(1))//'"')
    ...
    if (present(fail_if_missing)) then ; if (fail_if_missing) then
      if (.not.found) then
        ...
        call LND_error(FATAL,'Variable '//trim(varname)// &
                             ' found but not set in input files.')
      endif
    endif ; endif
  endif
  return
 1005 call LND_error(FATAL,'read_param_time: read error for time-type variable '//&
                           trim(varname)// ' parsing "'//trim(value_string(1))//'"')
    ...
    do count = 1, CS%param_data(ipf)%num_lines
      ...
      if (line(last1:last1) == achar(92).or.line(last1:last1) == "&") then
        ...
        if (count==CS%param_data(ipf)%num_lines .and. is_root_pe()) &
           call LND_error(FATAL, "LND_file_parser : the last line"// &
      ...
      if (iso>1) call LND_error(FATAL, "LND_file_parser : #override was found "// &
      ...
      if (iso==1) then ! % is first character means this is a close
        if (len_trim(blockName)==0 .and. is_root_pe()) call LND_error(FATAL, &
            'get_variable_line: An extra close block was encountered. Line="'// &
            trim(line(:last))//'"' )
        if (last>1 .and. trim(blockName)/=trim(line(2:last)) .and. is_root_pe()) &
            call LND_error(FATAL, 'get_variable_line: A named close for a parameter'// &
      ...
      if (inWrongBlock .and. .not. fullPathParameter) then
        if (index(" "//line(:last+1), " "//trim(varname)//" ")>0) &
          call LND_error(WARNING,"LND_file_parser : "//trim(varname)// &
      ...
      if (is_root_pe()) then
        if (.not. (found_define .or. found_undef .or. found_equals)) &
               call LND_error(FATAL, "LND_file_parser : the parameter name '"// &
                 trim(varname)//"' was found without define or undef."// &
                 " Line: '"//trim(line(:last))//"'"//&
                 " in file "//trim(filename)//".")
        if (found_define .and. found_undef) call LND_error(FATAL, &
                 "LND_file_parser : Both 'undef' and 'define' occur."// &
                 " Line: '"//trim(line(:last))//"'"//&
                 " in file "//trim(filename)//".")
        if (found_equals .and. (found_define .or. found_undef)) &
               call LND_error(FATAL, &
                 "LND_file_parser : Both 'a=b' and 'undef/define' syntax occur."// &
                 " Line: '"//trim(line(:last))//"'"//&
                 " in file "//trim(filename)//".")
        if (found_override .and. .not. (found_define .or. found_undef .or. found_equals)) &
               call LND_error(FATAL, "LND_file_parser : override was found "// &
      ...
      elseif (found_equals) then
        ...
        call LND_error(FATAL, "LND_file_parser (non-root PE?): the parameter name '"// &
      ...
      if (found_override .and. (oval >= max_vals)) then
        if (is_root_pe()) then
          if ((defined_in_line .neqv. defined) .or. .not. valueIsSame) then
            call LND_error(FATAL,"LND_file_parser : "//trim(varname)// &
...
subroutine closeParameterBlock(CS)
  ...
  if (associated(CS%blockName)) then
    block => CS%blockName
    if (is_root_pe().and.len_trim(block%name)==0) call LND_error(FATAL, &
...
!!  more than once, and UCLAND has a facility to check for unused lines
