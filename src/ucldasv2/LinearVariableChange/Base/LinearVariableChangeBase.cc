...
LinearVariableChangeFactory::LinearVariableChangeFactory(
  ...
  if (getMakers().find(name) != getMakers().end()) {
    oops::Log::error() << name
                  << " already registered in ucldasv2::LinearVariableChangeFactory."
                  << std::endl;
    ABORT("Element already registered in ucldasv2::LinearVariableChangeFactory.");
  }
  getMakers()[name] = this;
}
...
LinearVariableChangeBase * LinearVariableChangeFactory::create(
  ...
  if (jloc == getMakers().end()) {
    oops::Log::error() << id
                      << " does not exist in ucldasv2::LinearVariableChangeFactory."
                       << std::endl;
    ABORT("Element does not exist in ucldasv2::LinearVariableChangeFactory.");
  }
