...
VariableChangeFactory::VariableChangeFactory(const std::string & name) {
  if (getMakers().find(name) != getMakers().end()) {
    oops::Log::error() << name
                        << " already registered in ucldasv2::VariableChangeFactory."
                       << std::endl;
    ABORT("Element already registered in ucldasv2::VariableChangeFactory.");
  }
  getMakers()[name] = this;
}
...
VariableChangeBase * VariableChangeFactory::create(const Geometry & geom,
  ...
  if (jloc == getMakers().end()) {
    oops::Log::error() << id
              << " does not exist in ucldasv2::VariableChangeFactory." << std::endl;
    ABORT("Element does not exist in ucldasv2::VariableChangeFactory.");
  }
...
VariableChangeFactory::createParameters(const std::string &name) {
  ...
  if (it == getMakers().end()) {
    throw std::runtime_error(name +
                              " does not exist in ucldasv2::VariableChangeFactory");
  }
