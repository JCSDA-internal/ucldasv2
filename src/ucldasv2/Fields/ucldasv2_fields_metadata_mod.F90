...
!> Metadata for ucldasv2_fields
module ucldasv2_fields_metadata_mod
...
!!
!! Instances of these types are to be held by ucldasv2_fields_metadata
type, public :: ucldasv2_field_metadata
  character(len=:),  allocatable :: name     !< internal name used only by ucldasv2 code
...
! ------------------------------------------------------------------------------
!> A collection of ucldasv2_field_metadata types representing ALL possible fields
!! (state, increment, other derived) in ucldasv2. These are read in from a configuration file.
type, public :: ucldasv2_fields_metadata

  type(ucldasv2_field_metadata), private, allocatable :: metadata(:)

contains

  !> \copybrief ucldasv2_fields_metadata_create \see ucldasv2_fields_metadata_create
  procedure :: create => ucldasv2_fields_metadata_create

  !> \copybrief ucldasv2_fields_metadata_clone \see ucldasv2_fields_metadata_clone
  procedure :: clone  => ucldasv2_fields_metadata_clone

  !> \copybrief ucldasv2_fields_metadata_get \see ucldasv2_fields_metadata_get
  procedure :: get    => ucldasv2_fields_metadata_get
end type
...
!> Create the main ucldasv2_fields_metadata instance by reading in parameters from a
!! yaml file.
!!
!! See the members of ucldasv2_field_metadata for a list of valid options
!!
!! \throws abor1_ftn aborts if there are duplicate fields
!! \relates ucldasv2_fields_metadata_mod::ucldasv2_fields_metadata
subroutine ucldasv2_fields_metadata_create(self, filename)
  class(ucldasv2_fields_metadata), intent(inout) :: self
...
!!
!! \relates ucldasv2_fields_metadata_mod::ucldasv2_fields_metadata
subroutine ucldasv2_fields_metadata_clone(self, rhs)
  class(ucldasv2_fields_metadata), intent(inout) :: self
  class(ucldasv2_fields_metadata), intent(in) :: rhs !< metadata to clone \b from
...
function ucldasv2_fields_metadata_get(self, name) result(field)
  class(ucldasv2_fields_metadata), intent(in) :: self
